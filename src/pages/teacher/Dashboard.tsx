...
const TeacherDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const teacher = await SchoolService.getTeacherData(user.id);
      const profile = await SchoolService.getProfile(user.id);
      setTeacherData(teacher);
      setProfileData(profile);

      const [assignData, statsData] = await Promise.all([
        SchoolService.getTeacherAssignments(teacher.id),
        SchoolService.getTeacherStats(user.id)
      ]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <section className="lg:col-span-1 bg-surface rounded-2xl border border-slate-200 shadow-sm p-6 md:p-8">
          <div className="flex items-center justify-between mb-6">
            ...
            {/* Mobile context switcher if needed */}
          </div>
